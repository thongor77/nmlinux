...
## DT-22 — Topology : menu contextuel des nœuds muet sous Wayland natif — `QGraphicsItem.contextMenuEvent` remplacé par `customContextMenuRequested`

**Contexte :** Le clic droit sur un nœud de Topology (menu Ping/Port Scanner/SSH/…, via `HostActionMenu`) ne déclenchait jamais rien pour l'utilisateur, alors que la sélection au clic gauche fonctionnait (panneau Details à jour) — laissant penser que le clic droit n'avait jamais été implémenté. Diagnostic : le code de routage (`_NodeItem.contextMenuEvent` → `TopologyPage._show_node_menu` → `HostActionMenu`) était bien présent et fonctionnel — vérifié par injection directe d'un `QContextMenuEvent` sur le viewport en dehors de toute interaction utilisateur, qui déclenchait correctement la construction du menu. Le maillon manquant se situait donc en amont : sous Wayland natif (`QApplication().platformName() == 'wayland'`, pas XWayland), la plateforme ne délivre jamais l'événement `QContextMenuEvent` natif jusqu'au `QGraphicsView`/`QGraphicsScene`/item pour cet item `ItemIsMovable`+`ItemIsSelectable` — mécanisme plus fragile que le `contextMenuPolicy` standard des widgets Qt. `IpScannerPage`, qui offre le même menu sur un `QTableWidget` via `setContextMenuPolicy(CustomContextMenu)` + signal `customContextMenuRequested`, fonctionnait déjà correctement chez le même utilisateur, sur la même session.

**Décision :** Remplacer `_NodeItem.contextMenuEvent()` (supprimé) par le même mécanisme que `IpScannerPage` : `_TopoView.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)` + `customContextMenuRequested.connect(_on_context_menu)`, qui fait un hit-test direct via `itemAt(pos)` au lieu de dépendre de la propagation d'un `QContextMenuEvent` à travers la scène.

**Raisons :**
- Mécanisme déjà éprouvé et fonctionnel ailleurs dans le même fichier de code (`ip_scanner.py`) sur la session Wayland native de l'utilisateur — pas une supposition théorique.
- Évite toute dépendance à la synthèse d'événement contextuel par le QPA de la plateforme, qui s'est révélée peu fiable pour un `QGraphicsItem` movable/selectable sous Wayland natif.
- Cohérence : les deux points d'entrée de `HostActionMenu` (IP Scanner, Topology) utilisent désormais le même pattern d'ouverture.
