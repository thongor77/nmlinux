...
- **Terminal SSH — fix taille PTY perdue au démarrage** : signalé par l'utilisateur, dans une app plein écran côté distant (Claude Code CLI, htop, vim) le bas de l'écran affichait du texte résiduel qui ne se nettoyait que progressivement. Indice déterminant : redimensionner la fenêtre faisait disparaître le phénomène. Cause : `SshPage._start_session()` démarre la session avant que Qt ait layouté le nouvel onglet ; `SshWorker.resize()` jetait silencieusement la taille demandée si le PTY n'était pas encore spawné (race condition avec `QThread.start()`), le PTY restait bloqué à 24×80 côté noyau alors que `pyte`/`TerminalView` affichaient la vraie taille. Fix : `SshWorker` mémorise la taille en attente et l'applique juste après le spawn. Voir DT-17.
- Release GitHub (wheel + AppImage) : https://github.com/thongor77/nmlinux/releases/tag/v1.7.10. AUR toujours non mise à jour — dépôt en maintenance (revérifié en direct avant la release).

## 2026-08-03 — v1.7.11

- **Packaging Flatpak (KDE Linux)** : manifest local dans `packaging/flatpak/`, cible KDE Linux et toute distro immuable où Flatpak est le seul canal d'installation sérieux (AUR = Arch, `flake.nix` = NixOS). nmlinux shelle vers ~30 binaires système (`nmcli`, `pkexec`, `mount.cifs`, `ssh`, `nmap`, `xfreerdp`…) qui doivent agir sur le vrai NetworkManager/`/etc/hosts`/montages plutôt que dans le sandbox — chaque nom de binaire est shimmé vers `flatpak-spawn --host` via `host-bin/`, aucune réécriture des call sites existants. PySide6 vient de `io.qt.PySide.BaseApp//6.11` (Flathub) plutôt que du wheel PyPI — `flatpak-pip-generator` refuse explicitement ce dernier cas. Voir DT-18.
- Correctif découvert en implémentant : trois call sites (`smb_mount.py`, `file_transfer.py`, `hosts.py`) passaient des chemins sandbox-privés (`/tmp`, `/app`) à des commandes relayées sur l'hôte via `pkexec` — invisibles côté hôte. `nmlinux/core/flatpak_shim.py` les redirige vers `$XDG_CACHE_HOME/nmlinux/tmp`, seul répertoire que Flatpak fait pointer vers un vrai chemin hôte. No-op hors Flatpak.
- Distribution restée modeste : pas de soumission Flathub (sa review resserre en priorité `--filesystem=home` + `flatpak-spawn --host`, exactement ce que ce manifest utilise), pas de dépôt ostree auto-hébergé non plus. Un bundle `.flatpak` unique (`packaging/flatpak/build-bundle.sh`) joint à chaque release, comme l'AppImage.
- Build réel validé de bout en bout : `flatpak-builder --install` puis `flatpak install` depuis le fichier `.flatpak` généré, `flatpak run` sans erreur.
- Release GitHub (wheel + AppImage + bundle Flatpak) : https://github.com/thongor77/nmlinux/releases/tag/v1.7.11. AUR toujours non mise à jour — dépôt en maintenance (revérifié en direct avant la release).
